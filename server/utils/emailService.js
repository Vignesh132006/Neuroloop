    ...
    html: `
      <div style="font-family:sans-serif;background:#0a0a0a;color:#f5f0e8;
        ...
        <div style="background:#111111;border:1px solid rgba(255,255,255,0.06);
                    border-radius:8px;padding:16px;margin:16px 0;line-height:1.6;color:#a09880;white-space:pre-wrap;">${message}</div>
        <p style="color:#5a5040;font-size:0.75rem;margin-top:16px;">
          Sent from NeuroLoop Customer Support Widget
        </p>
      </div>
    `,
  };
...
async function sendAdminErrorEmail(userEmail, url, errorMessage, stack = "") {
  const mailOptions = {
    ...
    html: `
      <div style="font-family:sans-serif;background:#0a0a0a;color:#fca5a5;
                  padding:32px;border-radius:16px;max-width:550px;margin:auto;
                  border:1px solid rgba(239,68,68,0.3);">
        <h2 style="color:#ef4444;margin-bottom:16px;">Automatic System Error Alert</h2>
        <p style="margin:8px 0;color:#f5f0e8;"><strong>User Email:</strong> ${userEmail}</p>
        <p style="margin:8px 0;color:#f5f0e8;"><strong>Page URL:</strong> ${url}</p>
        <p style="margin:8px 0;color:#f5f0e8;"><strong>Error Message:</strong> ${errorMessage}</p>
        ${stack ? `
        <div style="background:#111111;border:1px solid rgba(239,68,68,0.15);
                    border-radius:8px;padding:16px;margin:16px 0;
                    font-family:monospace;font-size:0.85rem;color:#fca5a5;overflow-x:auto;white-space:pre-wrap;">${stack}</div>` : ''}
        <p style="color:#5a5040;font-size:0.75rem;margin-top:16px;">
          This is an automated error report generated by the frontend.
        </p>
      </div>
    `,
  };
